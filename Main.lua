...
AddCommand("invisible2", {"invis2"}, "Added by EthanChas", {3}, function(Caller)
...
AddConnection(CConnect(HttpLogs.Save.MouseButton1Click, function()
    ...
    local String =  format("Fates Admin HTTP for %s (%s)\n\n", GameName, os.date());
    local TimeSaved = gsub(tostring(os.date("%x")), "/", "-") .. " " .. gsub(tostring(os.date("%X")), ":", "-");
    local Name = format("fates-admin/webhooks/%s (%s).txt", GameName, TimeSaved);
    for i, v in next, GetChildren(HttpLogs.Frame.List) do
        if (not IsA(v, "UIListLayout")) then
            String = format("%s%s\n", String, v.Text);
        end
    ...
    Utils.Notify(LocalPlayer, "Saved", "HTTPS logs saved!");
end), Connections.UI, true);


AddCommand("dex", {}, "Added by EthanChas", {3}, function(Caller)
   print("Not done")
end)
